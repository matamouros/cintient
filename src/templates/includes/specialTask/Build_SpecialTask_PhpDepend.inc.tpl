...
{else}
        <div id="jdependChart"><embed type="image/svg+xml" src="{UrlManager::getForAsset($project_overviewPyramidFilename, ['pid' => $project_build->getProjectId(), 'bid' => $project_build->getId()])}" width="392" height="270" /></div>
        <div id="overviewChart"><embed type="image/svg+xml" src="{UrlManager::getForAsset($project_jdependChartFilename, ['pid' => $project_build->getProjectId(), 'bid' => $project_build->getId()])}" width="392" height="270" /></div>
{/if}
      </div>
{/capture}