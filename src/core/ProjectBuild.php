...
class ProjectBuild
{
    ...
    foreach ($xmls as $node) {
      ...
      if (!is_file($chartFile)) {
        if (!Chart::unitTests($chartFile, $methodsNames, $successes, $failures)) {
          SystemEvent::raise(SystemEvent::ERROR, "Chart file for unit tests was not saved. [PID={$this->getProject()->getId()}] [BUILD={$this->getId()}]", __METHOD__);
        } else {
          SystemEvent::raise(SystemEvent::INFO, "Generated chart file for unit tests. [PID={$this->getProject()->getId()}] [BUILD={$this->getId()}]", __METHOD__);
        }
      }
  ...
  {
    return md5($this->getProject()->getId() . $this->getId() . CINTIENT_PHPDEPEND_JDEPEND_CHART_FILENAME) . '.svg';
    //return $this->getReportsDir() . CINTIENT_PHPDEPEND_JDEPEND_CHART_FILENAME;
  }
  ...
  {
    return md5($this->getProject()->getId() . $this->getId() . CINTIENT_PHPDEPEND_SUMMARY_FILENAME) . '.xml';
    //return $this->getReportsDir() . CINTIENT_PHPDEPEND_SUMMARY_FILENAME;
  }
  ...
  {
    return md5($this->getProject()->getId() . $this->getId() . CINTIENT_PHPDEPEND_OVERVIEW_PYRAMID_FILENAME) . '.svg';
    //return $this->getReportsDir() . CINTIENT_PHPDEPEND_OVERVIEW_PYRAMID_FILENAME;
  }
  ...
  public function init()
  {
    ...
    // PHP_Depend files
    //
    if (!@copy($this->getProject()->getReportsWorkingDir() . CINTIENT_PHPDEPEND_JDEPEND_CHART_FILENAME, $this->getReportsDir() . $this->getJdependChartFilename())) {
      SystemEvent::raise(SystemEvent::ERROR, "Could not backup original PHP_Depend jdepend chart file [PID={$this->getProject()->getId()}] [BUILD={$this->getId()}]", __METHOD__);
    }
    if (!@copy($this->getProject()->getReportsWorkingDir() . CINTIENT_PHPDEPEND_OVERVIEW_PYRAMID_FILENAME, $this->getReportsDir() . $this->getOverviewPyramidFilename())) {
      SystemEvent::raise(SystemEvent::ERROR, "Could not backup original PHP_Depend overview pyramid file [PID={$this->getProject()->getId()}] [BUILD={$this->getId()}]", __METHOD__);
    }
    if (!@copy($this->getProject()->getReportsWorkingDir() . CINTIENT_PHPDEPEND_SUMMARY_FILENAME, $this->getReportsDir() . $this->getPhpDependSummaryFilename())) {
      SystemEvent::raise(SystemEvent::ERROR, "Could not backup original PHP_Depend summary file [PID={$this->getProject()->getId()}] [BUILD={$this->getId()}]", __METHOD__);
    }
    //
