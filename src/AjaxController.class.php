...
class AjaxController extends Sentient\Object
{
	...
	public function run()
	{
		if (!isset($GLOBALS['user']) || !($GLOBALS['user'] instanceof User)) {
					...
					($GLOBALS['subSection'] != 'registration' || $GLOBALS['settings'][SystemSettings::ALLOW_USER_REGISTRATION])))
			{
				...
				// TODO: improve exception and error
				throw new \Exception("Authentication is required on all ajax requests. [URI={$GLOBALS['uri']}]");
				exit;
			}
		}
	}
	...
	static public function dashboard_project()
	{
		...
		$smarty->setDebugging($GLOBALS['config']->valueForKey('smarty.debug'));
		$smarty->setForceCompile($GLOBALS['config']->valueForKey('smarty.force_compile'));
		$smarty->setCompileCheck($GLOBALS['config']->valueForKey('smarty.compile_check'));
		$smarty->setTemplateDir($GLOBALS['config']->valueForKey('smarty.template_dir'));
		$smarty->setCompileDir($GLOBALS['config']->valueForKey('smarty.compile_dir'));
